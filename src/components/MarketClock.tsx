...
export default function MarketClock() {
  ...
  return (
    <div className="row small nowrap" title={`Status do mercado americano: ${status}`}>
      <span className="row" style={{ gap: 6 }}><span className={`dot ${dot}`} /><strong>{status}</strong><span className="muted hide-mobile">{MARKET_STATUS_LABEL[status]}</span></span>
      <span className="num muted">{et} ET</span>
      <span className="num faint hide-mobile">{local} local</span>
    </div>
  );
}
